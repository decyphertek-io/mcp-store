#!/bin/bash
# Web-Search MCP Server PyInstaller Build Script (Linux) via Poetry

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
ENTRYPOINT="$SCRIPT_DIR/web.py"
OUTPUT_DIR="$PROJECT_ROOT"
BUILD_DIR="$SCRIPT_DIR/.build"

echo "==> Building web-search MCP binary using Poetry"
...
echo "==> Build complete: $OUTPUT_DIR/web.mcp"
